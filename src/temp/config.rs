...
use surrealdb::{iam::Level, opt::capabilities::Capabilities};
...
impl Config {
    ...
    pub fn user(mut self, user: surrealdb::opt::auth::Root<'_>) -> Self {
...
impl From<Config> for SurrealConfig {
    fn from(value: Config) -> Self {
        #[allow(unused)]
        let mut config = SurrealConfig::new()
            .set_notifications(value.notifications)
            .set_strict(value.strict)
        ...
        #[cfg(feature = "rustls")]
        config.rustls(value.tls_config);

        #[cfg(feature = "native-tls")]
        config.native_tls(value.tls_config);
